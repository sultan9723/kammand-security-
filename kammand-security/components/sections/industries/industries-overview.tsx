...
function IndustriesHeroVisual() {
  ...
  return <div className="industries-overview__hero-visual" aria-hidden="true"><div className="industries-overview__hero-orbits"><span /><span /><span /><span /></div><div className="industries-overview__hero-platform"><span /><span /><span /></div><div className="industries-overview__hero-panels">{panels.map((lines, index) => <div className={`industries-overview__hero-panel industries-overview__hero-panel--${index + 1}`} key={lines.join("-")}><IndustryIcon icon={icons[index]} /><span>{lines.map((line) => <span className="industries-overview__hero-panel-line" key={line}>{line}</span>)}</span></div>)}</div></div>;
}
...
export function IndustriesOverviewPage() {
  ...
  return (
    <main className="industries-overview" id="main-content">
      <section className="industries-overview__hero" aria-labelledby="industries-overview-title"><Container className="container-wide"><div className="industries-overview__hero-grid"><div className="industries-overview__hero-content"><SectionLabel as="p">Industries</SectionLabel><h1 id="industries-overview-title" aria-label="GRC and cybersecurity for high-accountability environments."><span>GRC and</span><span>cybersecurity for</span><span>high-accountability</span><span>environments.</span></h1><p className="text-body-large">Different sectors. Different risks.<br />One consistent standard: stronger governance,<br />clearer controls, measurable assurance.</p><div className="industries-overview__hero-actions"><Link className="ui-button ui-button--primary" href="/book">Book a Consultation <DirectionalArrow /></Link><Link className="ui-button ui-button--secondary" href="/services">Explore Services <DirectionalArrow /></Link></div></div><IndustriesHeroVisual /></div></Container></section>

      <section className="industries-overview__context" aria-labelledby="industry-introduction-title"><Container className="container-wide"><div className="industries-overview__context-band"><div><SectionLabel as="p">Context</SectionLabel><h2 id="industry-introduction-title">Industry context changes what needs attention first.</h2></div><p>KAMMAND does not treat every environment as the same. Governance, cyber risk, evidence, suppliers, privacy, resilience, and assurance need to be structured around the organization&apos;s operating model, obligations, and risk profile.</p></div></Container></section>

      <section className="industries-overview__categories" id="industry-categories" aria-labelledby="industry-categories-title"><Container><div className="industries-overview__section-header"><SectionLabel align="center" as="p">Industry Categories</SectionLabel><h2 id="industry-categories-title">Different environments. Different priorities.</h2><p>Focused advisory for the sectors where risk and accountability matter most.</p></div><div className="industries-overview__cards">{overviewIndustries.map((industry, index) => <Link className="industries-overview__card" href={industry.href} key={industry.href}><span className="industries-overview__card-icon"><IndustryIcon icon={industryIcons[index]} /></span><span className="industries-overview__card-number">{String(index + 1).padStart(2, "0")}</span><DirectionalArrow className="industries-overview__card-arrow" /><h3>{industry.title}</h3><p>{industry.description}</p><span className="industries-overview__card-tags">{industryTags[index].map((tag) => <span className="industries-overview__card-tag" key={tag}>{tag}</span>)}</span></Link>)}</div></Container></section>

      <section className="industries-overview__frameworks" aria-labelledby="industry-frameworks-title"><Container className="container-wide"><div className="industries-overview__framework-grid"><div className="industries-overview__framework-content"><SectionLabel as="p">Framework Context</SectionLabel><h2 id="industry-frameworks-title" aria-label="Industry does not automatically define applicability."><span>Industry does not</span><span>automatically define</span><span>applicability.</span></h2><p>We map the right frameworks to your obligations so you focus on what truly matters.</p><Link className="ui-button ui-button--secondary" href="/frameworks">View framework mapping <DirectionalArrow /></Link></div><div className="industries-overview__framework-rail">{frameworkSummaries.map((framework, index) => <Link href={framework.href} key={framework.href}><FrameworkIcon index={index} /><span>{framework.title}</span></Link>)}<span className="industries-overview__framework-axis" aria-hidden="true" /></div></div></Container></section>

      <section className="industries-overview__challenges" aria-labelledby="industry-challenges-title"><Container className="container-wide"><div className="industries-overview__challenges-panel"><div className="industries-overview__challenges-content"><SectionLabel as="p">Cross-Industry Challenges</SectionLabel><h2 id="industry-challenges-title">Recurring themes, not universal claims.</h2><p>These are advisory themes KAMMAND often considers when helping organizations structure governance, risk, controls, evidence, remediation, and assurance.</p></div><ul aria-label="Cross-industry advisory themes">{crossIndustryChallenges.map((challenge, index) => <li key={challenge}><ChallengeIcon index={index} /><span>{challenge}</span></li>)}</ul></div></Container></section>
      ...
      <section className="industries-overview__cta" aria-labelledby="industries-final-cta-title"><Container className="container-wide"><div className="industries-overview__cta-panel"><div><h2 id="industries-final-cta-title">Ready to discuss the right service path for your industry?</h2><p>Book a focused consultation or explore KAMMAND&apos;s service paths first.</p></div><div className="industries-overview__cta-actions" aria-label="Industries consultation actions"><Link className="ui-button ui-button--primary" href="/book">Book a Consultation <DirectionalArrow /></Link><Link className="ui-button ui-button--secondary" href="/services">Explore Services <DirectionalArrow /></Link></div><div className="industries-overview__cta-motif" aria-hidden="true"><span /><span /><span /></div></div></Container></section>
    </main>
  );
}
